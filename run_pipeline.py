...
def upsert_data(supabase, table_name, records, conflict_col):
    if not records: print(f"✅ INFO: No records for '{table_name}'."); return
...
def step_2_team_stats(supabase, year, team_map):
    ...
    records = [{'team_id': s.get('team_id'), 'wins': s.get('wins'), 'losses': s.get('losses'),
                'batting_avg': s.get('avg'), 'obp': s.get('onBasePct'), 'slugging': s.get('sluggingPct'),
                'runs_per_game': s.get('runsPerGame'), 'era': s.get('earnedRunAverage'),
                'whip': s.get('walksAndHitsPerInningPitched'), 'fielding_pct': s.get('fieldingPct'),
    ...
    for _, game in games_df.iterrows():
        try:
            ...
            predictions.append({'game_id': game['game_id'], 'pick_date': game['game_date'],
                                ...
                                'predicted_home_score': round(home_w_score / 10, 2),
                                'predicted_away_score': round(away_w_score / 10, 2),
...
# --- MAIN WORKFLOW ---
def main():
    print("🚀 Starting WagerIndex Daily Pipeline (v9.0 - Final Contract)...")
